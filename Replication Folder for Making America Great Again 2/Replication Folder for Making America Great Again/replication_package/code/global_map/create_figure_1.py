...
def create_stacked_maps(before_data_file, after_data_file, output_file):
    ...
    before_value_dict = dict(zip(before_data['Country'], before_data['Value']))
    after_value_dict = dict(zip(after_data['Country'], after_data['Value']))
...
# Run the script using paths inside the repository
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, os.pardir, os.pardir))
data_dir = os.path.join(REPO_ROOT, 'output')

before_csv = os.path.join(data_dir, 'output_map.csv')
after_csv = os.path.join(data_dir, 'output_map_retal.csv')
figure_path = os.path.join(data_dir, 'figure_1.png')

create_stacked_maps(before_csv, after_csv, figure_path)

# Clean up temporary files if they exist
for csv_path in (before_csv, after_csv):
